...
def get_fault_packages(db: Session, plant_id: int, hours: int = None, min_proba: float = 0.3, gap_minutes: int = 30):
    """
    Agrupa fallas consecutivas en paquetes.
    Dos alertas son la misma falla si el gap entre timestamps <= gap_minutes.
    """
    from datetime import timezone
    from backend.repositories.dashboard_repo import fetch_raw_faults_by_plant

    rows = fetch_raw_faults_by_plant(db, plant_id, hours=hours, min_proba=min_proba)
    if not rows:
        return []
    ...
    for row in rows:
        ts = row["ts"]
        # Normalizar a datetime con tz si viene como string
